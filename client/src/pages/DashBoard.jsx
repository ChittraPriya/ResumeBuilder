import { FilePenIcon, FilePenLineIcon, LoaderCircleIcon, PencilIcon, PlusIcon, TrashIcon, UploadCloud, XIcon } from 'lucide-react'
...
const DashBoard = () => {
  ...
  const colors = ["#9333ea", "#d97786", "#dc2626", "#0284c7", "#16a34a"]
  ...
  const loadAllResumes = async () => {
    try {
      const {data} = await api.get('/api/resumes',{headers: {Authorization: token}})
  ...
  return (
    <div>
      ...
      <p className='text-2xlnfont-medium mb-6 bg-gradient-to-r from-slate-600 to-sl;atew-700 bg-clip-text text-transparent sm:hidden'>Welcome, Chittra Priya</p>
      ...
      <div className='flex gap-4'>
        ...
        <button onClick={()=> setShowUploadResume(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-pink-500 hover:shadow-lg transition-all duration-300 cursor-pointer'>
          <UploadCloud className='size-11 transition-all duration-300 p-2.5 bg-gradient-to-br from-pink-300 to-pink-500 text-white rounded-full ' />
  ...
  {Array.isArray(allResumes) && allResumes.length > 0 ? (
    ...
    <p className="text-slate-500 text-sm">No resumes found.</p>
  )}
</div>
        ...
        {showCreateResume && (
          <form onSubmit={createResume} onClick={()=> setShowCreateResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e=> e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              <h2 className='rext-xl font-bold mb-4'>Create a Resume</h2>
        ...
        {showUploadResume && (
          <form onSubmit={uploadResume} onClick={()=> setShowUpdateResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e=> e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              ...
              <button disabled={isLoading} className='w-full py-2 bg-pink-600 text-white flex items-center justify-center gap-2 rounded hover:bg-pink-700 transition-colors'>
   ...
   {editResumeId && (
          <form onSubmit={editTitle} onClick={()=> setEditResumeId('')} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e=> e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              <h2 className='rext-xl font-bold mb-4'>Edit Resume Title</h2>
              <input onChange={(e)=>setTitle(e.target.value)} value={title} type="text" placeholder='Enter Resume Title' className='w-full px-4 py-2 mb-4 focus:borser-pink-600 ring-pink-600 required:' />
